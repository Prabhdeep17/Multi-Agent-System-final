...
def writer_agent(state: PolicyState) -> PolicyState:
    ...
    messages = [
        SystemMessage(content=(
            ...
            "- NO CROSS-DOCUMENT WEAVING: Never combine disjointed terms from different documents into a single sentence to make it sound cohesive. You MUST NOT cite multiple different documents in the same sentence. One document per sentence maximum.\n\n"
            "## FORMAT RULES:\n"
...
def reviewer_agent(state: PolicyState) -> PolicyState:
    ...
    messages = [
        SystemMessage(content=(
            ...
            "## CHECK 1.6 — NO NARRATIVE WEAVING & CONCLUDING SENTENCES:\n"
            "Verify the Writer did not weave disjointed facts from separate documents into a single 'cohesive' sentence. If you see a sentence that ends with MULTIPLE different citations (e.g. '(Source: A.docx; Source: B.docx)'), it is a hallucinated narrative weave! You MUST delete that sentence entirely.\n\n"
